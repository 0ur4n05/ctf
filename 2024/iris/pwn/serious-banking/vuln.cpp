...
void interface() {
    while(true) {
        ...
        printf(separator);					// format string 100% just need to write into separator
        ...
        const int selection = fgetc(stdin) - static_cast<int>('0');		// what the fuck is a static cast
        fgetc(stdin);
        ...
        switch (selection) {
            case 1: {
                ...
                std::cin.getline(account_name, 80);				// getting the name
                for (size_t i = 0; i < 80; i++) {			// this fucker is blocking me from leaking
                    if (account_name[i] == '\n') {
                        account_name[i] = '\0';
                        break;
                    }
                }
		// well we fucking cant leak something from here
                account_name[79] = '\0';

                accounts[account_count].id = id_counter++;		
                accounts[account_count].active = true;
                accounts[account_count].name = account_name;			// a fucking account name
            ...
            case 2: {
                ...
                size_t number;
                std::cin >> number;							// here we can fuck some shit
                if (std::cin.fail()) {
                ...
                if (number >= account_count) {					// well not for long
                    printf("That account does not exist.");
                    break;
                }

                const Account acc = accounts[number];				 // here we be printing shit
            ...
            case 5: {
                ...
                size_t number;					// getting some account type of shit 
                std::cin >> number;
                if (std::cin.fail()) {
                    printf("Invalid Input.");
                    exit(EXIT_FAILURE);
		...
		////// weeeeeeeeeeeeeeeird fucking piece of code
                char* name_ptr = name + strlen(name);			// whop whop whop we fucking ball
...
int main() {
    ...
    debug_log = new char[2900];				// layrzini ila wlad l97ab
